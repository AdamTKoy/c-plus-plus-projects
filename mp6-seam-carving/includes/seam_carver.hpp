...
private:
   ...
   * Add any helper methods you may need
   */
  int FindHorizontalMin(int* values[]) const;
  void HorizontalTrack(int** values, int* track, int best) const;
  int BestHoriz(int** col_vals) const;

  int FindVerticalMin(int** values) const;
  void VerticalTrack(int** values, int* track, int best) const;

  int BestValueTwo(int same_idx, int other_idx) const;
  int BestValueThree(int same_idx, int other_idx_1, int other_idx_2) const;
};
